...
{
	if (nrhs != 2|| !mxIsDouble(prhs[0])) mexErrMsgTxt("Usage : fast_rotate((double)image,(double)ang)");
	...
	source=(double *)mxGetData(prhs[0]);
	...
	for(color=0;color<dim;color++)
	{
		...
		for(y=0;y<height;y++)
		{
			...
			for(x=0;x<width;x++)
			{
				X=(int)(w2 + (x-w2)*ca + (y-h2)*sa-0.5); // Source X  % I changed from +0.5 to -0.5 
...
// 				Y=(int)(h2 - (x-dw2)*sa + (y-dh2)*ca); // Source Y- without cropping
				...
				dest[index_Height+x] = (X<0 || Y<0 || X>=width || Y>=height)?0.0:source[index_Color+Y*width+X];
			}
		}
	}
}
