                 ...
                 int nrhs, const mxArray *prhs[]) /* Input variables */
{
    ...
    /*Parsing inputs*/
    if(nrhs != 2 || mxGetNumberOfDimensions(prhs[0]) != 2 || !mxIsDouble(prhs[0]))
    ...
    for(k = 1; k <= N; k++){
        r = (double)k;
        *(ptrKreal+k-1) = sqrt(r)*cos(4.0*log(r));
        *(ptrKim+k-1) = sqrt(r)*sin(4.0*log(r));
    }
