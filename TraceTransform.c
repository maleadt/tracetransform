                 ...
                 int nrhs, const mxArray *prhs[]) /* Input variables */
{
    ...
    /*mxArray variables*/
    mxArray *Input_data[3], *Rotated_image[2], *Input_Tfunct[2],
        *Input_Stacking[3], *Output_Stacking[1], *StackSinog[3],
        *Nonorm_CircusF[1];
    ...
    /*Parsing inputs*/
    if (nrhs != 5 || mxGetNumberOfDimensions(prhs[0]) != 2 ||
        !mxIsDouble(prhs[0]))
    ...
    Numb_rotations =
        (int)(360.0 / (angle_intrvl * 4.0)); /*Number of rotations*/
    ...
    for (k = 0; k < Numb_rotations; k++) { /*Loop to rotate the image*/
        ...
        *angle = (double)(angle_intrvl *
                          k); /* Angle in which the image will be rotated*/

        mexCallMATLAB(1, &Rotated_image[0], 3, Input_data,
                      "imrotate"); /*Rotating the input image*/
        mexCallMATLAB(1, &Rotated_image[1], 1, &Rotated_image[0],
                      "rotflip") /*Rotated, flipped, and transposed image */;

        // Loop to compute the vector shifts of the rotated and its transposed
        for (n = 0; n < 2; n++) {

            // Find the vector shift with weighted median
            //
            // NOTE: only do this if more than 1 functional, or the only one
            // isn't Radon (which doesn't need this)
            if (Numb_Tfunct != 1 || (int)T_codes[0] != 0) {
                mexCallMATLAB(1, &Input_Tfunct[1], 1, &Rotated_image[n],
                              "vecWM");
            } else {
                Input_Tfunct[1] = 0;
            }

            // Storing the rotated image and the shift vector to apply T
            // functional
            memcpy(mxGetPr(Input_Tfunct[0]), mxGetPr(Rotated_image[n]),
                   sizeof(double) * N * M);

            // Loop to apply the T-functional
            for (c = 0; c < Numb_Tfunct; c++) {
                int tfunctional = (int)*(T_codes + c);
                Tfunct[11] = (char)((int)base + tfunctional);

                // Compute the T-functional
                if (tfunctional == 0)
                ...
                *(stack_idx + 3) = (double)Numb_rotations;

                // Stores the T-functional to compose the sinogram
                mexCallMATLAB(1, Output_Stacking, 3, Input_Stacking,
                              "Stacking");
                memcpy(mxGetPr(Input_Stacking[2]), mxGetPr(Output_Stacking[0]),
                       sizeof(double) * N * Nang * Numb_Tfunct);
            }
        }
    }
